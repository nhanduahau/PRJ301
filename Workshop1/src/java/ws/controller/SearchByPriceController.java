...
public class SearchByPriceController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            DAO dao = new DAO();
            List<Mobile> listMobile = dao.getMobileListByPrice(min, max);
            if (listMobile.size() > 0) {
                request.setAttribute("LIST_MOBILE_BY_PRICE", listMobile);
                url = SUCCESS;
            }
        } catch (Exception e) {
            request.setAttribute("ExNum", "Please enter a number");
